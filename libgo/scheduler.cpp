...
uint32_t Scheduler::Run(int flags)
{
    ...
    // 涓嬩竴娆imer鎴杝leeper瑙﹀彂鐨勬椂闂存绉掓暟, 浼戠湢鎴栭樆濉炵瓑寰匢O浜嬩欢瑙﹀彂鐨勬椂闂翠笉鑳借秴杩囪繖涓��
    ...
    if (flags & erf_idle_cpu) {
        if (!run_task_count && ep_count <= 0 && !tm_count && !sl_count) {
            if (ep_count == -1) {
                // 姝ょ嚎绋嬫病鏈夋墽琛宔poll_wait, 浣跨敤sleep闄嶄綆绌鸿浆鏃剁殑cpu浣跨敤鐜�
                ++info.sleep_ms;
...
uint32_t Scheduler::DoRunnable(bool allow_steal)
{
    ...
    if (!do_count && !done_count && allow_steal) {
        ...
        if (thread_count > 1) {
            ...
            if (r == info.thread_id)    // 涓嶈兘閫夊埌褰撳墠绾跨▼
