...
namespace co {
    ...
    inline int libgo_poll(struct pollfd *fds, nfds_t nfds, int timeout, bool nonblocking_check)
    {
        ...
        if (nonblocking_check) {
            // 鎵ц涓�娆￠潪闃诲鐨刾oll, 妫�娴嬪紓甯告垨鏃犳晥fd.
        ...
        if (!triggered) {
            // 娌℃湁绔嬪嵆瑙﹀彂, 鍗忕▼鍒囨崲鑷崇瓑寰呯姸鎬�, 绛夊緟琚玡poll鍞ら啋鎴栬秴鏃�
            // set timer
            if (timeout > 0)
        ...
        uint32_t event, int timeout_so, ssize_t buflen, Args && ... args)
{
    ...
    FdCtxPtr fd_ctx = FdManager::getInstance().get_fd_ctx(fd);
    if (!fd_ctx || fd_ctx->closed()) {
        errno = EBADF;  // 宸茶close鎴栨棤鏁堢殑fd
        return -1;
    }

    if (!fd_ctx->is_socket())   // 闈瀞ocket, 鏆備笉HOOK. 浠ヤ繚闅滄枃浠秄d璇诲啓姝ｅ父
        return fn(fd, std::forward<Args>(args)...);
...
retry_intr_fn:
    ...
    if (n == -1) {
        if (errno == EAGAIN || errno == EWOULDBLOCK) {
            ...
            goto retry;     // 浜嬩欢瑙﹀彂 OR epoll鎯婄兢鏁堝簲
        }
        else if (errno == EINTR)
            goto retry_intr_fn;
    }
        ...
        fd_set *exceptfds, struct timeval *timeout)
{
    ...
    // 鎵ц涓�娆￠潪闃诲鐨剆elect, 妫�娴嬪紓甯告垨鏃犳晥fd.
    fd_set rfs, wfs, efs;
    FD_ZERO(&rfs);
    FD_ZERO(&wfs);
    FD_ZERO(&efs);
...
int dup2(int oldfd, int newfd)
{
    ...
    FdManager::getInstance().close(newfd, false);   // 妯℃嫙dup2鍐呴儴鏈兘hook鍒扮殑close.
